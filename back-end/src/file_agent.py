...
class FileAgent:
    ...
    def getFiles(self, input):
        prompt = f"""I need you to determine whether you need to consult the data stored in 
        some files to answer this prompt: {input}. Here you have the atributes that store 
        each file: {self.atributos}. It is extremely important that you strictly only answer 
        with the name of the files you consider more likely would be needed to consult, 
        separated by commas. Include the .csv in the name. If you think you don't need to 
        consult any file answer 'NO'. If the petition is something like 'Qué le pasa a Juan García',
        you definetely will have, at least, to consider the file resumen_pacientes.csv and the file
        resumen_lab_iniciales.csv too.
        """
